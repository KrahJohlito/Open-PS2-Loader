...
void drawCoverFlow(struct menu_list *menu, struct submenu_list *item, config_set_t *config, struct theme_element *elem)
{
    ...
    if (item->prev != NULL) {
        ...
        if (game->prev != NULL)
            ...
            game = item;
            while (game->next)
                game = game->next; // last_game
        }
    } else {
        game = item;
        while (game->next)
            game = game->next;
        game = game->prev; // last_game->prev.. no pointer for last game though
    }
    ...
    for (i = 0; i < COVERFLOW_COUNT; i++) {
        ...
        mutable_image_t *gameCover = (mutable_image_t *)elem->extended;
        GSTEXTURE *texture = getGameImageTexture(gameCover->cache, menu->item->userdata, &game->item);
        if (!texture || !texture->Mem) {
        ...
        if (game->next != NULL)
            ...
            game = menu->item->submenu; // first_game
    }
}
